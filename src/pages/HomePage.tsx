...
import {
  ...
  AlertCircle, ChevronRight, Activity, BrainCircuit
} from 'lucide-react';
  ...
  const onSubmit = async (values: SearchValues) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast.success('Búsqueda DNIGPT completada');
      } else {
      ...
      <div className="py-8 md:py-10 lg:py-12 min-h-screen flex flex-col items-center relative">
        ...
        <div className="w-full flex flex-col items-center justify-center space-y-12">
          ...
          >
            <div className="flex justify-center mb-6">
              <div className="p-4 rounded-2xl bg-indigo-600 shadow-glow animate-float relative overflow-hidden group">
                <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-500" />
                <BrainCircuit className="w-10 h-10 text-white relative z-10" />
              </div>
            </div>
            <h1 className="text-5xl md:text-7xl font-display font-black tracking-tighter text-slate-900 dark:text-white">
              DNI<span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-blue-600 shadow-glow-sm">GPT</span>
            </h1>
            <p className="text-slate-500 dark:text-slate-400 max-w-lg mx-auto text-lg leading-relaxed font-medium">
              Motor avanzado de inteligencia para el rastreo de registros públicos argentinos.
              <span className="block text-indigo-500 dark:text-indigo-400 text-sm font-bold mt-2 tracking-widest uppercase">Powered by DNIGPT Engine</span>
            </p>
          </motion.div>
          {/* Search Box */}
          <motion.div
          ...
          >
            <Card className="border-none shadow-soft glass dark:glass-dark relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-600 via-blue-500 to-cyan-500" />
              <CardContent className="pt-8">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" aria-label="Búsqueda Inteligente DNIGPT">
                  <div className="space-y-2">
                    <div className="relative group">
                      ...
                      <Input
                        ...
                        className="pl-11 pr-10 h-14 text-xl font-mono bg-white/50 dark:bg-slate-950/50 border-slate-200 dark:border-slate-800 focus:ring-indigo-500 transition-all rounded-xl shadow-inner"
                  ...
                  <Button
                    type="submit"
                    disabled={loading}
                    className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-lg rounded-xl transition-all shadow-primary hover:shadow-glow-lg active:scale-95"
                  >
                    {loading ? (
                      <div className="flex items-center gap-2">
                        <Loader2 className="w-5 h-5 animate-spin" />
                        <span>Analizando con DNIGPT...</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <BrainCircuit className="w-5 h-5" />
                        <span>Consultar Inteligencia</span>
                      </div>
                    )}
                  </Button>
                </form>
                    ...
                    <Card key={sIdx} className="border-none shadow-soft glass dark:glass-dark overflow-hidden flex flex-col h-full min-h-[400px] hover:shadow-glow transition-shadow duration-300">
                      <CardHeader className="bg-slate-100/30 dark:bg-white/5 border-b border-slate-200 dark:border-slate-800 py-4">
                        <div className="flex items-center justify-between">
                          ...
                          >
                            {source.status === 'success' ? 'EXTRAÍDO' : 'SIN REGISTRO'}
                          </Badge>
                        </div>
                      </CardHeader>
                          ...
                          <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {source.items.map((item, iIdx) => (
                              <motion.div
                                ...
                                key={iIdx}
                                className="px-5 py-4 flex items-start gap-4 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-colors group cursor-default"
                              >
                                <ChevronRight className="w-3.5 h-3.5 text-indigo-400 mt-1 flex-shrink-0 group-hover:translate-x-1 transition-transform" />
                ...
                >
                  <Dialog open={showPremium} onOpenChange={setShowPremium}>
                    <DialogTrigger asChild>
                      <Button className="h-16 px-14 rounded-full bg-gradient-to-r from-indigo-600 via-blue-600 to-violet-600 hover:from-indigo-700 hover:via-blue-700 hover:to-violet-700 text-white font-black text-lg shadow-glow-lg hover:scale-105 transition-all group border-none">
                        <Sparkles className="w-6 h-6 mr-3 group-hover:rotate-12 transition-transform" />
                        GENERAR INFORME DNIGPT PREMIUM
                      </Button>
                    </DialogTrigger>
                    <DialogContent className="sm:max-w-md border-none glass dark:glass-dark shadow-2xl p-0 overflow-hidden">
                      <div className="bg-indigo-600 p-8 text-white text-center">
                        ...
                        <DialogTitle className="text-2xl font-black">DNIGPT Premium Report</DialogTitle>
                        <DialogDescription className="text-indigo-100 mt-2 font-medium">
                          Consolide inteligencia financiera y social avanzada en un único PDF auditado.
                        </DialogDescription>
                      </div>
                      <div className="p-8 space-y-6">
                        <div className="space-y-4">
                          {[
                            'Análisis Crediticio BCRA (Situación 1-5)',
                            'Cheques rechazados & Juicios Activos',
                            'Nexo con Sociedades & Directorios',
                            'Vínculos de Telemarketing & Localización',
                            'Certificación de Veracidad DNIGPT'
                          ].map((feature, fIdx) => (
                        ...
                        <div className="flex items-end justify-between border-t border-slate-100 dark:border-slate-800 pt-6">
                          ...
                          <Badge className="bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 font-black">ENTREGA AI</Badge>
                        </div>
                        <DialogFooter className="flex-col sm:flex-col gap-3">
                          ...
                          >
                            <CreditCard className="w-5 h-5 mr-3" /> Pagar con Seguridad
                          </Button>
                          <p className="text-[10px] text-center text-slate-400 font-medium">
                            Encriptación SSL de 256 bits • DNIGPT PRO SECURE
                          </p>
                        </DialogFooter>
                      </div>
                    </DialogContent>
        ...
        <footer className="w-full mt-auto pt-20 pb-8 text-center">
          <div className="flex flex-col items-center gap-4">
            <div className="h-px w-24 bg-gradient-to-r from-transparent via-indigo-200 dark:via-indigo-800 to-transparent mb-4" />
            <p className="text-sm text-slate-400 font-bold tracking-tight">
              © {new Date().getFullYear()} DNIGPT PRO • MOTOR DE BÚSQUEDA INTELIGENTE v2.6.0
            </p>
            <div className="flex flex-wrap justify-center gap-x-8 gap-y-2 text-[10px] text-slate-400 uppercase tracking-[0.2em] font-black">
              <span className="hover:text-indigo-500 cursor-pointer transition-colors">Términos</span>
              <span className="hover:text-indigo-500 cursor-pointer transition-colors">Privacidad</span>
              <span className="hover:text-indigo-500 cursor-pointer transition-colors">DNIGPT API</span>
              <span className="hover:text-indigo-500 cursor-pointer transition-colors">Soporte</span>
            </div>
          </div>
        </footer>
